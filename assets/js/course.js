...
class Sidebar {
  ...
  constructor(selector) {
    ...
    if (!this.el) {
      console.warn(
        "Could not initialize sidebar. No sidebar found at selector",
        selector,
      );
      return;
    }
  ...
  async getLesson(id) {
    ...
    try {
      ...
      if (lesson.id != id) {
        throw new Error(
          `Tried to get content for lesson ${id}, but instead got ${lesson.id}`,
        );
      }
  ...
  async handleLessonSuccess(lesson, id) {
    ...
    window.history.pushState(
      { lesson: lesson.title, lessonId: lesson.id },
      "",
      url,
    );
  ...
  insertContent(html, builder, id) {
    ...
    if (html === "" || html == null) {
      if (entry && entry.status == "loading") {
        ...
        content =
          '<div class="lighter-fetch-err">Error loading lesson. Please try again</div>';
      } else {
        content = "";
      }
    }
    ...
    contentArea.innerHTML =
      '<div class="lighter-lesson-wrap">' + content + "</div>";

    if (entry.status == "success") {
      window.scrollTo({
        top: contentArea.getBoundingClientRect().y + window.scrollY - 20,
        behavior: "smooth",
      });
      ...
      if (complete) {
        ...
        if (container && container.classList.contains("lighter-hidden"))
          container.classList.remove("lighter-hidden");
      }
    }
  }
}
