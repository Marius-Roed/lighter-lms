...
export class CourseService {
  ...
  async moveLesson(
    ...
    const grabOrder = (lessons: Lesson[]) =>
      lessons.map((l) => ({ key: l.key, sortOrder: l.sortOrder }));

    const fromTopic = this.course.topics.find((t) => t.key === fromTopicKey);
    const toTopic =
      this.course.topics.find((t) => t.key === toTopicKey) ?? fromTopic;
    ...
    try {
      const toData = {
        topic_key: toTopic.key,
        reorder: toTopic.lessons.map((l) => ({
          id: l.id,
          key: l.key,
      ...
      if (isDifferent)
        fromData = {
          topic_key: fromTopic.key,
          reorder: fromTopic.lessons.map((l) => ({
            id: l.id,
            key: l.key,
            sort_order: l.sortOrder,
          })),
      ...
      const newOrder = await this.#api.updateLessonOrder(toData, fromData);

      newOrder.forEach((t) => this.course.updateTopic(t));
    } catch (e) {
      console.error(e);
  ...
  createLesson(
    topicKey: string,
    data: { title: string; lesson_type?: string },
  ): void {
  ...
  insertTopic(
    ...
    if (!existingTopic)
      throw new Error(`Cannot find node to insert adjacent topic on`);
    ...
    const placeholder: TopicData = {
      ...
      sortOrder:
        position === "before"
          ? existingTopic.sortOrder
          : existingTopic.sortOrder + 1,
      lessons,
    };

    this.course.topics.forEach((t) => {
      t.sortOrder =
        t.sortOrder > existingTopic.sortOrder ? t.sortOrder + 1 : t.sortOrder;
  ...
  createTopic(title: string, lessons: LessonData[] = []): void {
    ...
    this.#api
      .createTopic(new Topic(placeholder))
      .then((real) => {
  ...
  moveTopic(fromIndex: number, toIndex: number) {
    ...
    this.#api
      .moveTopic(fromTopic.key, reordered)
      .then((res) => {
        res.forEach((n) => {
  ...
  deleteTopic(topicKey: string) {
    ...
    this.#api.deleteTopic(topicKey).catch(() => {
      if (this.course) this.course.topics = snapshot;
      // TODO: Toast failure.
    });
  }
}
