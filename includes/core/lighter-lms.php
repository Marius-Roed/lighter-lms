...
final class Lighter_LMS
{
     ...
     * User access object
     */
    public readonly User_Access $user;
    ...
    public function maybe_update_schema(): void
    {
        ...
        if (
            version_compare(
                (string) get_option(Lighter_LMS_Schema::VERSION_OPTION, "0"),
                "2",
                ">=",
            )
        ) {
    ...
    public function check_update(object $transient): object
    {
        ...
        if (
            ...
            $cached["last_check"] >
                current_time("timestamp") - 0.5 * HOUR_IN_SECONDS
        ) {
            if (
                version_compare($cached["remote_version"], $installed_ver, ">")
            ) {
        ...
        if (is_wp_error($releases)) {
            error_log(
                "LighterLMS update check: API Error - " .
                    $releases->get_error_message(),
            );
            return $transient;
        }
        ...
        if ($releases_code !== 200) {
            error_log(
                "LighterLMS update check: Non-200 response (" .
                    $releases_code .
                    ")",
            );
            return $transient;
        ...
        if (
            !isset($latest["tag_name"]) ||
            !isset($latest["assets"]) ||
            empty($latest["assets"])
        ) {
            error_log(
                "LighterLMS update check: Invalid GitHub response - no tag or assets",
            );
            return $transient;
        }
        ...
        if (!$package_url) {
            error_log(
                "LighterLMS update check: No valid or matching zip found in release",
            );
            return $transient;
        }
    ...
    public function get_plugin_info(
        ...
        if (false === $c_info) {
            ...
            if (
                is_wp_error($releases) ||
                wp_remote_retrieve_response_code($releases) !== 200
            ) {
                return $result;
            }
        ...
        $plugin_info = (object) [
            ...
            "description" =>
                "Lighter LMS is a lightweight LMS plugin for WordPress.",
    ...
    private function _parse_changelog(string $body): array
    {
        $sections = [
            ...
            "changelog" => $body,
            "installation" =>
                "Extract and upload the lighter-lms folder to /wp-content/plugins/. Activate in WordPress.",
        ];

        if (
            preg_match_all('/^## (.+)$/m', $body, $headers, PREG_OFFSET_CAPTURE)
        ) {
            foreach ($headers[1] as $index => $header) {
                $section_name = strtolower(
                    str_replace(" ", "_", trim($header[0])),
                );
                $section_name = preg_replace("/[^a-z0-9_]/", "", $section_name);

                $start =
                    $headers[0][$index][1] + strlen($headers[0][$index][0]) + 1;
                $end = isset($headers[0][$index + 1])
                    ? $headers[0][$index + 1][1]
                    : strlen($body);
