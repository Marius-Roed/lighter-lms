...
class API {
	...
	public $namespace         = 'lighterlms/v1';
	...
	public function __construct() {
		add_action( 'rest_api_init', array( $this, 'register_lesson_routes' ) );
		add_action( 'rest_api_init', array( $this, 'register_topic_routes' ) );
		add_action( 'rest_api_init', array( $this, 'register_course_routes' ) );
		add_action( 'rest_api_init', array( $this, 'register_import_export_routes' ) );

		add_filter( 'rest_' . lighter_lms()->course_post_type . '_query', array( $this, 'course_rest' ), 10, 2 );
		add_filter( 'rest_prepare_' . lighter_lms()->lesson_post_type, array( $this, 'ensure_fields' ), 10, 3 );
		add_filter( 'rest_prepare_' . lighter_lms()->course_post_type, array( $this, 'ensure_fields' ), 10, 3 );
		add_filter( 'rest_prepare_user', array( $this, 'ensure_user' ), 10, 3 );
	...
	public function create_lesson( $req ) {
		...
		if ( ! isset( $args['parent_topic'] ) ) {
			return new \WP_Error( 'creation_failed', 'Cannot create a Lesson without a parent topic', array( 'status' => 400 ) );
		}
		...
		return rest_ensure_response( array_merge( get_post( $new_id, ARRAY_A ), array( 'permalink' => admin_url( "post.php?post={$new_id}&action=edit" ) ) ) );
	}
	...
	public function get_lesson( $req ) {
		...
		if ( ! $lesson ) {
			return new \WP_Error( 'not_found', "Lesson with ID {$id} not found", array( 'status' => 404 ) );
		}
		...
		if ( ! $user->check_lesson_access( $id, $course_id ) ) {
			return new \WP_Error( 'access_denied', 'You do not have access to this lesson', array( 'status' => 403 ) );
		}

		if ( $only_content ) {
			$supported_builders = array(
			...
			if ( $builder && isset( $supported_builders[ $builder ] ) ) {
				$func    = array( Lesson_Content::class, $supported_builders[ $builder ] );
		...
		if ( $lesson->post_type !== lighter_lms()->lesson_post_type ) {
			return new \WP_Error( 'lighter_lms_forbidden', "Post with ID {$id} is not of post type: " . lighter_lms()->lesson_post_type, array( 'status' => 403 ) );
		}

		return rest_ensure_response( array( 'lesson' => $lesson ) );
	}
	...
	public function delete_lesson( $req ) {
		...
		if ( ! $key ) {
			return new WP_Error( 'bad_request', 'A key needs to be supplied to delete a lesson', array( 'status' => 400 ) );
		}
		...
		if ( ! $query->have_posts() ) {
			return new \WP_Error( 'not_found', "Lesson with Key {$key} not found", array( 'status' => 404 ) );
		} elseif ( $query->post_count > 1 ) {
			return new \WP_Error(
				'server_error',
				"Found multiple lessons with key {$key}",
				array(
					'status'      => 400,
					'queried_obj' => $query,
				)
		...
		if ( ! $deleted ) {
			return new \WP_Error( 'delete_failed', "Could not delete lesson with ID {$post_id}", array( 'status' => 500 ) );
		}
	...
	public function create_topic( $req ) {
		...
		if ( ! isset( $args['course_id'] ) ) {
			return new \WP_Error( 'creation_failed', 'Cannot create a topic outside a course. No course id passed.', array( 'status' => 400 ) );
		}
		...
		if ( ! $created_id ) {
			return new \WP_Error( 'server_error', 'Error creating topic', array( 'status' => 500 ) );
		}
	...
	public function search_topics( $req ) {
		...
		if ( empty( $search_term ) ) {
			...
			if ( ! $topics ) {
				return new WP_Error( 'no_topics', 'No topics were found', array( 'status' => 404 ) );
			}

			$topics = array_map( array( $topic_db::class, 'normalise_for_rest' ), $topics );
	...
	public function update_topic( $req ) {
		...
		return rest_ensure_response( array( 'topic' => $topic ) );
	}
	...
	public function delete_topic( $req ) {
		...
		if ( ! $success ) {
			return new WP_Error( 'server_error', 'Internal server error. Could not delete topic', array( 'status' => 500 ) );
		}
	...
	public function register_course_routes() {
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'start_import_job' ),
		...
		register_rest_route(
			$this->namespace,
			'/import/(?P<job_id>import_[a-f0-9]{13})',
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'start_import_job' ),
				'permission_callback' => fn() => current_user_can( 'edit_users' ),
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'get_import_job' ),
				'permission_callback' => fn() => current_user_can( 'edit_users' ),
				'args'                => array(
					'job_id' => array(
						'required'          => true,
		...
		register_rest_route(
			...
			array(
				'methods'             => 'PUT,PATCH',
				'callback'            => array( $this, 'update_import_job' ),
				'permission_callback' => fn() => current_user_can( 'edit_users' ),
				'args'                => array(
					'job_id' => array(
						'required'          => true,
	...
	public function start_import_job( $req ) {
		...
		if ( empty( $file ) && empty( $_POST ) && $_SERVER['CONTENT_LENGTH'] > 0 ) {
			return new WP_Error( 'file_too_large', 'File exceeds server size limits.', array( 'status' => 413 ) );
		}

		if ( ! $opts || ! $file ) {
			return new WP_Error( 'no_file', 'No file or options found.', array( 'status' => 400 ) );
		}
		...
		if ( ! $handle ) {
			return new WP_Error( 'not_found', "Could not find file at {$file_path}", array( 'status' => 404 ) );
		}
		...
		return rest_ensure_response( array( 'job' => $init_state ) );
	}
	...
	public function get_import_job( $req ) {
		...
		if ( ! $job ) {
			return new WP_Error( 'not_found', 'Could not find job with id: ' . $job_id, array( 'status' => 404 ) );
		}
	...
	public function update_import_job( $req ) {
		...
		} elseif ( isset( $body['paused'] ) && $body['paused'] ) {
			$job['status'] = 'paused';
		} elseif ( isset( $body['resume'] ) && $body['resume'] ) {
			$job['status'] = 'running';
		} else {
			return new WP_Error( 'bad_request', "Request should contain a true of either 'cancel', 'paused' or 'resume'.", array( 'status' => 400 ) );
		}
	...
	public function get_courses_tags( $object, $field_name, $req ) {
		return wp_get_post_terms( $object['id'], 'course-tags', array( 'fields' => 'names' ) );
	}
	...
	public function register_import_export_routes() {
		...
		register_rest_field(
			...
			'columns',
			array(
				'get_callback' => array( $this, 'get_courses_columns' ),
				'schema'       => null,
			),
			10,
			3
		...
		register_rest_field(
			...
			'bulk_actions',
			array(
				'get_callback' => array( $this, 'get_courses_bulk_actions' ),
				'schema'       => null,
			),
			10,
			3
		...
		register_rest_field(
			...
			'course_title',
			array(
				'get_callback' => array( $this, 'get_courses_title' ),
				'schema'       => null,
			)
		);
	}
	...
	public function ensure_fields( $response, $post, $request ) {
		...
		if ( empty( $response->data['title'] ) ) {
			$response->data['title'] = array( 'rendered' => get_post_field( 'post_title', $post, 'raw' ) ?: '' ); // NOTE: raw value as Svelte will escape it.
		}
