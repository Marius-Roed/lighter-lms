...
class Course_Post extends Post_Type
{
    public function __construct()
    {
        ...
        add_action("admin_post_lighter_lms_add_topic", [$this, "add_topic"]);
            ...
            foreach ($_POST["topics"] as $topic) {
                ...
                if (isset($topic["lessons"])) {
                    foreach ($topic["lessons"] as $lesson) {
                        if ($lesson["dirty"]) {
                            Lesson_Post::save_from_course($lesson, $topic);
                        }
                    }
                }
            }
    ...
    protected function _save_settings(\WP_Post $post, array $args): void
    {
        ...
        $show_icons = wp_validate_boolean(
            $args["show_icons"] ??
                lighter_lms()->defaults()->course_show_lesson_icons,
        );
        $show_progress = wp_validate_boolean(
            $args["show_progress"] ??
                lighter_lms()->defaults()->course_show_progress,
        );
        $sync_prod_img = wp_validate_boolean($args["sync_prod_img"] ?? true);
        $slug = $args["slug"]
            ? sanitize_post_field("post_name", $args["slug"], $post->ID, "raw")
            : $post->post_name;
        ...
        if ($is_restricted) {
            ...
            if (!$user_access->check_course_access($post->ID)) {
                ...
                wp_die(
                    "access denied",
                    __(
                        "Please purchase the course or log in to view.",
                        "lighterlms",
                    ),
                    ["response" => 403],
                );
