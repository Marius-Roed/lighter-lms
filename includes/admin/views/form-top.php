...
<div class="lighter-header">
	<div class="inner">
		<div class="content">
			...
			<?php if (in_array($post_type, lighter_lms()->post_types)) : ?>
				<div class="title-wrapper col">
					<label id="title-prompt-text" class="screen-text-reader" for="title"><?= esc_html($title_placeholder) ?></label>
					<input form="post" type="text" name="post_title" value="<?= esc_attr($post_title) ?>" id="title" class="title" spellcheck="true" autocomplete="off" placeholder="Course name" />
				</div>
				<div>
					<a href="#" class="lighter-btn transparent"><?= lighter_icon("plus") . esc_html($add_new) ?></a>
				</div>
				<input type="hidden" form="post" name="lighter_nonce" value="<?= wp_create_nonce($post_type . '_fields') ?>" />
			<?php endif; ?>
		</div>
		<div class="actions row">
			...
			<?php endif; ?>
			<button form="post" class="lighter-btn" id="save-post"><?= $publish_btn === "save" ? lighter_icon("save") . esc_html__($publish_btn) : esc_html__($publish_btn) ?></button>
		</div>
	</div>
</div>
