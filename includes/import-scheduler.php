...
class Import_Scheduler
{
	...
	public static function schedule_batch($job_id, $force = false)
	{
		...
		if (function_exists('as_schedule_single_action')) {
			if ($force || ! \as_next_scheduled_action(self::HOOK, [$job_id], self::GROUP)) {
				\as_schedule_single_action(time(), self::HOOK, [$job_id], self::GROUP);
			}
			return;
		}

		// If not, use wp cron.
		if ($force || ! wp_next_scheduled(self::HOOK, [$job_id])) {
			wp_schedule_single_event(time(), self::HOOK, [$job_id], self::GROUP);
		}
	}
	...
	public static function clear_schedule($job_id)
	{
		if (function_exists('as_unschedule_action')) {
			\as_unschedule_action(self::HOOK, [$job_id], self::GROUP);
		}
