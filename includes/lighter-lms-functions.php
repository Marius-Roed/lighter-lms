...
if ( ! function_exists( 'lighter_include' ) ) {
	function lighter_include( $relpath = '', $args = array() ) {
...
if ( ! function_exists( 'lighter_view' ) ) {
	function lighter_view( $path, $args = array() ) {
...
if ( ! function_exists( 'lighter_attrify' ) ) {
	...
	function lighter_attrify( $value, $seperator = '-' ) {
		return strtolower( preg_replace( array( '/\s/', '/_/', '/-/' ), $seperator, $value ) );
	}
}
		...
		<li class="lighter-topic" data-key="<?php echo esc_attr( $item['key'] ); ?>">
			...
			<ul class="course-lessons open">
				...
				foreach ( $item['lessons'] as $lesson ) {
					...
					if ( in_array( $lesson['id'], $data['owned']['lessons'] ?? array() ) || current_user_can( 'manage_options' ) ) {
						printf(
		...
		return array_map(
			function ( $post ) {
				...
				return array(
					...
					'tags'         => wp_get_post_terms( $post->ID, 'course-tags', array( 'fields' => 'names' ) ),
					'type'         => $post->post_type,
				);
			},
			$posts
...
if ( ! function_exists( 'lighter_postlist_js_obj' ) ) {
	function lighter_postlist_js_obj( $post_type ) {
		...
		$all_tags = get_terms( array( 'taxonomy' => 'course-tags' ) );
		$all_tags = array_map(
			fn( $tag ) => array(
				'id'       => $tag->term_id,
				'name'     => $tag->name,
