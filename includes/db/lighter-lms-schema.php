...
final class Lighter_LMS_Schema {
	...
	public function __construct( \wpdb $wpdb ) {
		...
		$this->topic_lessons_table = $this->db->prefix . lighter_lms()->topic_lessons_table;

		$this->legacy_topics_table  = $this->db->prefix . 'lighter_topics';
		$this->legacy_lessons_table = $this->db->prefix . 'lighter_lessons';
	}

	public function maybe_upgrade(): bool|\WP_Error {
	...
	protected function run_upgrade(): bool|\WP_Error {
		...
		if ( $this->_table_exists( $this->legacy_topics_table ) || $this->_table_exists( $this->legacy_lessons_table ) ) {
	...
	private function _install_current_schema(): bool|\WP_Error {
		...
		if ( ! $this->_table_exists( $this->topic_lessons_table ) ) {
			return new \WP_Error(
				...
				__( 'Could not create the LMS topic lessons table.', 'lighterlms' ),
				array(
	...
	protected function maybe_add_topic_foreign_key(): bool|\WP_Error {
		...
		if ( $this->_constraint_exists( $this->topic_lessons_table, $constraint ) ) {
			return true;
		}
	...
	private function _validate_legacy_data(): bool|\WP_Error {
		...
		if ( $null_topics > 0 ) {
			return new \WP_Error(
				...
				__( 'Mirgation stopped. Legacy rows do not have topics assigned', 'lighterlms' ),
				array(
		...
		if ( $this->_table_exists( $this->legacy_topics_table ) ) {
			...
			if ( $orphaned_topic_refs > 0 ) {
				return new \WP_Error(
					...
					__( 'Migration stopped. Legacy rows contain deleted topic ids', 'lighterlms' ),
					array(
		...
		if ( $duplicate_pairs > 0 ) {
			return new \WP_Error(
				...
				__( 'Migration stopped. Duplicate lesson on topic.', 'lighterlms' ),
				array(
