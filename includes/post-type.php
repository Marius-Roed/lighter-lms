...
abstract class Post_Type
{
    ...
    public function __construct($post_type_slug)
    {
        ...
        add_filter("get_user_option_screen_layout_" . $this->post_type, [
            $this,
            "screen_layout",
        ]);
    ...
    {
        $name =
            strpos("-tags", $name) == false
                ? strtolower($name) . "-tags"
                : strtolower($name);
    ...
    abstract protected function _save_settings(
        \WP_Post $post,
        array $args,
    ): void;
    ...
    final public static function js_objects(
        array $obj,
        string $screen_id,
    ): array {
        if ("lighter_lessons" === $screen_id) {
            $obj["lesson"]["settings"] = lighter_lms_get_lesson_settings();
        }

        if ("lighter_courses" === $screen_id) {
            $obj["course"] = Course_Post::_generate_object();
            $obj["course"]["settings"] = lighter_lms_get_course_settings();
        }
