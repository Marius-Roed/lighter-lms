...
class Topics
{
	...
	{
		$allowed = ['ID', 'post_id', 'topic_key', 'title', 'sort_order', 'created_at', 'updated_at'];
	...
	public function search($search_term, $args = [])
	{
		...
		if (current_user_can('edit_others_posts')) {
			$allowed_status = ['publish', 'private', 'future', 'pending', 'draft', 'trash'];
		} else if (current_user_can('read_private_posts')) {
		...
		$res = array_map(fn($topic) => [
			'title' => $topic['post_title'],
			'ID' => $topic['post_id'],
			'topics' => array_map(fn($val) => [explode("^", $val)[0] => explode("^", $val)[1]], explode("\x1F", $topic['topics'])),
