...
class Post_Types {
	...
	public function __construct() {
		...
		add_action( 'do_meta_boxes', array( $this, 'remove_submitdiv' ) );
		add_action( 'save_post_' . $this->course_post_type, array( $this, 'save_course' ), 10, 2 );
		add_action( 'save_post_' . $this->lesson_post_type, array( $this, 'save_lesson' ), 10, 2 );
		add_action( 'edit_form_after_title', array( $this, 'no_script' ) );
		add_action( 'admin_post_add_topic', array( $this, 'add_topic' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'scripts' ) );
		add_action( 'manage_' . $this->course_post_type . '_posts_custom_column', array( $this, 'course_custom_columns' ), 10, 2 );

		add_action( 'template_redirect', array( $this, 'course_access' ) );

		add_filter( 'get_user_option_screen_layout_' . $this->course_post_type, array( $this, 'screen_layout' ), 10, 1 );
		add_filter( 'post_class', array( $this, 'post_class' ), 10, 3 );
		add_filter( 'manage_' . $this->course_post_type . '_posts_columns', array( $this, 'course_columns' ) );
		add_filter( 'rest_' . $this->course_post_type . '_query', array( $this, 'course_rest_query' ), 20, 2 );
		add_filter( 'lighter_lms_admin_object', array( $this, 'js_objects' ), 10, 2 );
	...
	public function register_course_post_type() {
		...
		$args = array(
			...
			'register_meta_box_cb'  => array( $this, 'course_mbs' ),
		);
	...
	public function save_course( $post_id, $post ) {
		...
		if ( $post->post_name !== $settings['slug'] ) {
			...
			remove_action( 'save_post_' . $this->course_post_type, array( $this, 'save_course' ) );
			wp_update_post(
				array(
					'ID'        => $post_id,
	...
	public function course_access() {
		...
		if ( $is_restricted ) {
			...
			if ( ! $user_access->check_course_access( $post->ID ) ) {
				wp_die( 'access denied', 'Please purchase the lesson or log in to view.', array( 'response' => 403 ) );
			}
		}
	}
	...
	public function register_lesson_post_type() {
		...
		$args = array(
			...
			'register_meta_box_cb' => array( $this, 'lesson_mbs' ),
		);
	...
	public function save_lesson( $post_id, $post ) {
		...
		if ( isset( $settings['slug'] ) && ! empty( $settings['slug'] ) ) {
			...
			if ( $slug !== get_post_field( 'post_name', $post, 'raw' ) ) {
				// WARN: Maybe not the best way, but not removing the action causes recursion.
				remove_action( 'save_post_' . $this->lesson_post_type, array( $this, 'save_lesson' ) );
				wp_update_post(
					array(
						'ID'        => $post_id,
						'post_name' => $slug,
